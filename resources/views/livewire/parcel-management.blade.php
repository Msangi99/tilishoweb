<div class="space-y-6" x-data="{
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/30">
            <div class="flex items-center gap-3">
                ...
                <select wire:model.live="perPage" class="bg-white border border-slate-200 text-slate-700 text-sm rounded-xl focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 block w-20 p-2 outline-none transition-all font-bold">
            ...
            <div class="flex-1 flex justify-end">
                <div class="relative w-full md:w-64">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search parcels, tracking ID..." class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 outline-none transition-all placeholder:text-slate-300">
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b bg-white">
                        <th class="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400">Tracking ID / QR</th>
                        <th class="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400">Sender</th>
                        <th class="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400">Receiver</th>
                        <th class="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400">Destination</th>
                        <th class="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-slate-400">Amount</th>
                            ...
                            <td class="px-6 py-4" data-no-click>
                                <div class="flex items-center gap-4">
                                    <div @click="showQrModal = true; qrCodeUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={{ $parcel->tracking_number }}'; qrTrackingId = '{{ $parcel->tracking_number }}'" 
                                        ...
                                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data={{ $parcel->tracking_number }}" alt="QR Code" class="w-full h-full object-contain pointer-events-none">
                                        <div class="absolute inset-0 bg-blue-600/0 group-hover:bg-blue-600/5 transition-all pointer-events-none"></div>
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="text-xs font-black text-slate-900">{{ $parcel->tracking_number }}</span>
                                        <span class="text-[10px] text-slate-400 font-bold">{{ $parcel->created_at->format('M d, H:i') }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[11px] text-slate-500 font-mono">{{ $parcel->sender_phone }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm font-bold text-slate-900">{{ $parcel->receiver_name }}</span>
                                    <span class="text-[11px] text-slate-500 font-mono">{{ $parcel->receiver_phone }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2 text-wrap">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-400 flex-shrink-0"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
                                    <span class="text-sm font-bold text-slate-700 line-clamp-1">{{ $parcel->destination }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm font-black text-slate-900 text-nowrap">TZS {{ number_format($parcel->amount) }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $statusClasses = [
                                ...
                                @endphp
                                <span class="px-2.5 py-1 text-[10px] font-black uppercase tracking-widest rounded-lg border {{ $statusClasses[$parcel->display_status] ?? 'bg-slate-50 text-slate-600 border-slate-200' }}">
                            ...
                            <td colspan="7" class="px-6 py-24 text-center">
                                <div class="flex flex-col items-center">
                                    <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-6 border border-slate-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-300"><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>
                                    </div>
                                    <h4 class="text-lg font-bold text-slate-800">No parcels found</h4>
                                    <p class="text-sm text-slate-500 mt-1 max-w-xs mx-auto">Start by registering your first parcel to see it listed here.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-show="showQrModal" 
        ...
        <div @click.away="showQrModal = false" 
            ...
            <button @click="showQrModal = false" class="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-900 transition-colors">
            ...
            <div class="text-center">
                <div class="mb-8">
                    ...
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Scan for tracking info</p>
                </div>
                
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-inner inline-block mx-auto mb-8">
                    <img :src="qrCodeUrl" alt="QR Code" class="w-64 h-64 object-contain">
                </div>

                <button @click="showQrModal = false" class="w-full py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all shadow-xl shadow-slate-900/20">
                    Close Preview
                </button>
            </div>
        </div>
    ...
    <div x-show="showDetails" 
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div x-show="showDetails" @click="showDetails = false; $wire.closeDetails()" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-slate-900/60 backdrop-blur-md transition-opacity"></div>
            ...
            <div x-show="showDetails" 
                ...
                <div class="bg-slate-900 p-10 text-white relative">
                    <button @click="showDetails = false; $wire.closeDetails()" class="absolute top-8 right-8 text-white/60 hover:text-white transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                    </button>
                    <div class="flex items-start gap-6">
                        <div class="p-4 bg-white/10 backdrop-blur-xl rounded-[2rem] border border-white/20">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data={{ $viewingParcel->tracking_number }}&color=ffffff&bgcolor=1a2234" alt="QR" class="w-20 h-20 object-contain">
                        </div>
                        <div>
                            <span class="px-3 py-1 bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest rounded-full mb-3 inline-block">Parcel Details</span>
                            <h3 class="text-3xl font-black tracking-tight mb-1">{{ $viewingParcel->tracking_number }}</h3>
                            <p class="text-white/60 text-sm font-medium">Registered on {{ $viewingParcel->created_at->format('F d, Y \a\t H:i') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-10 space-y-10">
                    <div class="grid grid-cols-2 gap-10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <p class="text-lg font-black text-slate-900 leading-none mb-1">{{ $viewingParcel->sender_name }}</p>
                                <p class="text-sm text-slate-500 font-mono">{{ $viewingParcel->sender_phone }}</p>
                            </div>
                        </div>
                        <!-- Receiver -->
                        <div class="space-y-4">
                            ...
                            <div>
                                <p class="text-lg font-black text-slate-900 leading-none mb-1">{{ $viewingParcel->receiver_name }}</p>
                                <p class="text-sm text-slate-500 font-mono">{{ $viewingParcel->receiver_phone }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-10">
                        <!-- Route -->
                        <div class="space-y-4">
                            <h4 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Journey</h4>
                            <div class="flex items-center gap-3">
                                <div class="text-center">
                                    <p class="text-sm font-black text-slate-900">{{ $viewingParcel->origin }}</p>
                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Origin</p>
                                </div>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-slate-300"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                                <div class="text-center">
                                    <p class="text-sm font-black text-slate-900">{{ $viewingParcel->destination }}</p>
                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Destination</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Info -->
                        <div class="space-y-4">
                            <h4 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Information</h4>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <p class="text-sm font-black text-slate-900">TZS {{ number_format($viewingParcel->amount) }}</p>
                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Amount</p>
                                </div>
                                <div>
                                    <div class="flex flex-col">
                    ...
                    <div class="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                        <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Internal Description</h4>
                        <p class="text-sm text-slate-600 font-medium italic">"{{ $viewingParcel->description ?: 'No description provided.' }}"</p>
                    </div>
